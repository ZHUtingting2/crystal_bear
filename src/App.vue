...
import { GLTFLoader } from "three/examples/jsm/loaders/GLTFLoader.js";//导入加载器
...
onMounted(() => {
  ...
  bgTexture.mapping = THREE.EquirectangularReflectionMapping;//纹理反射效果
  ...
  //加载小熊模型
  const gltfLoader = new GLTFLoader();//使用加载器
  gltfLoader.load('/assets/model/bear.gltf', (gltf) => {
    console.log(gltf);
    const model = gltf.scene.children[0];
    model.material = new THREE.MeshPhongMaterial({//这个材质像水晶一样，有折射效果
      color: 0xffffff,
      envMap: bgTexture,//环境的纹理，可以折射出环境
